...
async def all_products(db: Annotated[Session, Depends(get_db)]):
    products = db.scalars(select(Product).join(Category).where(Product.is_active,
                                                               Category.is_active,
                                                               Product.stock > 0)).all()
    if not products:
        raise HTTPException(
...
async def product_by_category(db: Annotated[Session, Depends(get_db)], category_slug: str):
    ...
    products = db.scalars(select(Product).where(Product.category_id.in_(categories_and_subcategories),
                                                Product.is_active,
                                                Product.stock > 0)).all()
    return products
...
async def delete_product(db: Annotated[Session, Depends(get_db)], product_slug: str):
    product_delete = db.scalar(select(Product).where(Product.slug == product_slug, Product.is_active))
